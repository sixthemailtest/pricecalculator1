    ...
    <div className="App">
          ...
          <div className="short-stay-section" style={{ 
            ...
            {selectedRoomsForShortStay.length > 0 && (
              <div className="selected-rooms-container" style={{ 
                ...
                {(() => {
                  const selectedRooms = selectedRoomsForShortStay.map(roomNumber => 
                    [...rooms.groundFloor, ...rooms.firstFloor].find(r => r.number === roomNumber)
                  ).filter(room => room); // Filter out any undefined rooms
                  
                  // Group 1: Non-smoking, Non-Jacuzzi rooms
                  const nonSmokingNonJacuzziRooms = selectedRooms.filter(room => 
                    !room.smoking && room.type !== 'jacuzzi'
                  );
                  
                  // Group 2: Smoking, Non-Jacuzzi rooms
                  const smokingNonJacuzziRooms = selectedRooms.filter(room => 
                    room.smoking && room.type !== 'jacuzzi'
                  );
                  
                  // Group 3: Non-smoking, Jacuzzi rooms
                  const nonSmokingJacuzziRooms = selectedRooms.filter(room => 
                    !room.smoking && room.type === 'jacuzzi'
                  );
                  
                  // Group 4: Smoking, Jacuzzi rooms
                  const smokingJacuzziRooms = selectedRooms.filter(room => 
                    room.smoking && room.type === 'jacuzzi'
                  );
                  
                  // Sort each group by bed type: Queen, King, Queen 2 Beds
                  const sortByBedType = (rooms) => {
                    return [...rooms].sort((a, b) => {
                      const bedOrder = { 'Queen': 1, 'King': 2, 'Queen 2 Beds': 3 };
                      return bedOrder[a.beds] - bedOrder[b.beds];
                    });
                  };
                  
                  const sortedNonSmokingNonJacuzziRooms = sortByBedType(nonSmokingNonJacuzziRooms);
                  const sortedSmokingNonJacuzziRooms = sortByBedType(smokingNonJacuzziRooms);
                  const sortedNonSmokingJacuzziRooms = sortByBedType(nonSmokingJacuzziRooms);
                  const sortedSmokingJacuzziRooms = sortByBedType(smokingJacuzziRooms);
                  
                  // Render a row of rooms
                  const renderRoomRow = (rooms, label) => {
                  ...
                  return (
                    <div>
                      {renderRoomRow(sortedNonSmokingNonJacuzziRooms, 'Non-Smoking Rooms')}
                      {renderRoomRow(sortedSmokingNonJacuzziRooms, 'Smoking Rooms')}
                      {renderRoomRow(sortedNonSmokingJacuzziRooms, 'Non-Smoking Jacuzzi Rooms')}
                      {renderRoomRow(sortedSmokingJacuzziRooms, 'Smoking Jacuzzi Rooms')}
                    </div>
                  );
                })()}
              </div>
      ...
      <LoginModal 
        ...
        onClose={() => {
          ...
          setPendingRoomAction(null);
          setPendingAction(null); // Also clear pendingAction when closing modal
        }}
        onLogin={handleLogin}
      />
